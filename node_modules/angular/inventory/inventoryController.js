angular.module('inventoryApp', []).controller('inventoryCtrl', function($scope, $http) {
	...
	$scope.add = function() {
		if($scope.addToCollection == null ) {
			...
			$http.post("http://localhost:3000/api/inventoryapi/insert?collection=" + $scope.addToCollection + "&name=" + $scope.addName + "&type=" + $scope.addType + "&cost=" + $scope.addCost)
	...
	$scope.remove = function() {
		$http.post("http://localhost:3000/api/inventoryapi/delete?collection=boat&name=SUP4")
					.success(function(response){
						$scope.warning = response;
					});
	};
